...
public class ThirdPersonOrbitCamBasicMobile : MonoBehaviour 
{
	...
	void Awake()
	{
		//player = GamePlayManager.gameManager.player;
	...
	private int NonAnalogScreenMove()
	{
	...
	void Update()
	{
		...
		int touchIndex = NonAnalogScreenMove();

		// Get camera movement via touchscreen.
		if (camTouchscreen &&  touchIndex >= 0)
		{
			if (EventSystem.current.IsPointerOverGameObject())
            {
				return;
            }
			angleH += (Input.GetTouch(touchIndex).deltaPosition.x / 10f) * horizontalAimingSpeed;
			angleV += (Input.GetTouch(touchIndex).deltaPosition.y / 10f) * verticalAimingSpeed;
		}
